/*
    Actividad: Efecto Blurring (Desenfoque) - Multiprocesadores

    Jonatan Emanuel Salinas Avila       A01731815
    8/Oct/2021

    Este programa genera una imagen desenfocada en escala de grises (con efecto blurring) a partir de
    una imagen .bmp . Para ello, de define el tamanio de la mascara a utilizar.
    Ademas, se utiliza OpenMP para paralelizar el proceso de generar la nueva imagen.
*/
...
int blurring(){
    ...
    outputImage = fopen("toad_blurring.bmp","wb");    //Imagen transformada
    long ancho;
    long alto;
    ...
    printf("Cree los dos arrays bien.\n");
    ...
    for(int rows=0; rows<alto; rows++){
        for(int cols=0; cols<ancho; cols++){
            ...
            cont_imag_temp++;
                                       ...
                                       //Grises
    printf("Ya llene el array imagen_temp\n");
    ...
    printf("Voy a llenar output_temp\n");
    #pragma omp parallel
    {
    ...
    printf("Voy a generar outputImage ahora si\n");
